...
def calibrate_camera():
    ...
    if gray is None:
        print("No images could be processed. Calibration cannot be performed.")
        return None, None
...
def send_socket_data(socket, frame, roi, detected_class):
    ...
    y = max(0, min(y, 720))
    signal = 1 if detected_class == 'Filled_Slot' else 2
    if signal not in [1, 2]:
        print("Error: Invalid signal value")
        return
    (socket.send((str(signal) + ',' + str(round(x)) + ',' + str(round(y))).encode()))
    time.sleep(0.2)
...
def main():
    ...
    if mtx is None or dist is None:
        print("Calibration was unsuccessful. Cannot proceed with undistortion.")
        return

    while True:
        ret, frame = cap.read()
        frame = cv2.undistort(frame, mtx, dist, None, mtx)  # undistort the frame before processing it
